...
import com.weiwu.youxuanfinanciala.dmin.BuildConfig;
...
public class Logger {
     ...
     * @param args   the info of arguments passed to the formatter. If there are
    ...
    private static String buildPrivateMessage(Throwable throwable, String format, Object... args) {
        ...
        MESSAGE_BUFFER.delete(0, MESSAGE_BUFFER.length()); // 清空
        try {
            MESSAGE_BUFFER.append("{ ");

            MESSAGE_BUFFER.append(getSimpleStackTrace());

            MESSAGE_BUFFER.append((args == null) ? format : String.format(Locale.CHINA, format, args));
        } catch (Exception e) {
            MESSAGE_BUFFER.append("buildPrivateMessage exception " + e.getMessage());
        }

        if (DEBUG_MORE_DETAIL_STACK_INFO) {
            MESSAGE_BUFFER.append("\n");
            MESSAGE_BUFFER.append(getMoreDetailStackTrace());
            MESSAGE_BUFFER.append("\n");
        }
        if (throwable != null) {

            MESSAGE_BUFFER.append(throwable.toString());
            MESSAGE_BUFFER.append("\n");
        }
        MESSAGE_BUFFER.append(" } \n");


        return MESSAGE_BUFFER.toString();
    }
